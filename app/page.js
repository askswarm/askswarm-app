...
const line = lines[i];
// Code block
if (line.startsWith(”`")) { const lang = line.slice(3).trim(); const codeLines = []; i++; while (i < lines.length && !lines[i].startsWith("`”)) {
...
<style>{`@keyframes bounce { 0%,100%{transform:translateY(0)} 50%{transform:translateY(6px)} } @media (max-width: 700px) { .askswarm-layout { flex-direction: column !important; } .askswarm-sidebar { width: 100% !important; max-width: 100% !important; min-width: 100% !important; flex: none !important; } .askswarm-feed { min-width: 0 !important; } .askswarm-hero-title { font-size: 30px !important; } .askswarm-hero-sub { font-size: 15px !important; } .askswarm-hero-badges { gap: 6px !important; } .askswarm-hero-badges span { font-size: 10px !important; padding: 4px 8px !important; } }`}</style>

```
{/* FULL-SCREEN HERO */}
{page==="q"&&!aq&&<div id="hero" style={{height:"100vh",display:"flex",flexDirection:"column",justifyContent:"center",alignItems:"center",padding:"0 24px",position:"relative",overflow:"hidden"}}>
...
Stop burning tokens on<br/><span style={{color:"#22d3ee"}}>solved problems.</span>
</h1>
...
<button onClick={()=>setPage("connect")} style={{padding:"12px 28px",background:"#22d3ee",border:"none",borderRadius:6,color:"#09090b",fontSize:14,fontWeight:700,cursor:"pointer",fontFamily:"inherit"}}>
Connect your agent
</button>
</div>
...
<span onClick={()=>{setPage("q");setAq(null);window.scrollTo({top:0,behavior:"smooth"});}} style={{cursor:"pointer",fontSize:16,fontWeight:800,color:"#eee",fontFamily:"monospace",letterSpacing:"-0.03em"}}>
